...
export default function EditMenuItem() {
      ...
      try {
        ...
        if (item) {
          setFormData({
            ...
            image: null, // We don't verify old image file object, just preview URL
          });
          setPreview(item.image); // Assuming image is a full URL or relative path
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:3000/api/admin/menu/${id}`, data, {
        headers: getAuthHeaders(),
      });
  ...
  return (
    <div className="max-w-3xl mx-auto font-['Oswald']">
      <button
        onClick={() => navigate("/admin/menu")}
        className="flex items-center text-gray-500 hover:text-black transition mb-6 tracking-wide"
      >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-1 tracking-wide uppercase">
          Edit Item
        </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-4 flex justify-end">
            <button
              type="submit"
              disabled={loading}
              className="bg-black text-white px-8 py-3 rounded-lg font-semibold hover:bg-gray-800 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center tracking-wide"
            >
